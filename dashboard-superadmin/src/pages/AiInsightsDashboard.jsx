...
export default function AiInsightsDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchInsights = async () => {
      if (!isMounted) return;
      try {
        // Add timestamp to URL to prevent browser/HTTP caching
        const url = `/api/ai/insights?_t=${Date.now()}`;
        const res = await axios.get(url, {
          // Prevent caching to ensure fresh data every 5 seconds
          headers: {
        ...
        // CRITICAL: Reset last sync timer on successful fetch (same pattern as Dashboard, Alerts, Tickets)
        // This must happen to show "Last sync: 0 sec ago" after each fetch
        setLastSyncSeconds(0);
      } catch (error) {
        // Don't reset insights on error, just log it
        if (!isMounted) return;
        console.warn('AI insights fetch error:', error);
        // Don't call resetInsights() here - keep previous data visible
      }
    };
    ...
    // Set up polling interval (every 5 seconds) - same pattern as Dashboard, Alerts, Tickets
    const fetchInterval = setInterval(() => {
      // Always fetch - same pattern as other pages
      fetchInsights();
      // Check service status in parallel (non-blocking)
      checkAiService();
    }, 5000);

    // Set up timer interval (every 1 second for last sync display) - same pattern as other pages
