...
export default function Tickets() {
  ...
  const fetchTickets = useCallback(async () => {
    if (!selectedDistrict) {
      console.log('No district selected, skipping ticket fetch')
      return
    }
    try {
      console.log('Fetching tickets for district:', selectedDistrict)
      // Fetch ALL tickets for the district (not filtered by status) to show accurate counts for all tabs
      const params = {
  ...
  if (selectedVillage !== 'all') {
    filteredTickets = filteredTickets.filter(t => {
      ...
      const selectedVillageName = filteredVillages.find(v => (v.id || v.name) === selectedVillage)?.name?.toLowerCase() || ''
    ...
    return (
      <div className="tickets-page">
        <div className="page-header">
          ...
          <p className="page-subtitle">Select a district to view tickets</p>
        </div>
  ...
  return (
    <div className="tickets-page">
      ...
      <div className="page-header">
        <div>
          <h1>Issues (Tickets) • {selectedDistrict}</h1>
          <p className="page-subtitle">Track and manage maintenance tickets for {selectedDistrict}</p>
